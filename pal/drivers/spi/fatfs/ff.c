...
//#include "..\job manager\spi_master_drv.h"  /* Include file for user provided disk functions */
#include "..\bus manager\CxSpiBusManager.h"

FATFS *FatFs;			/* Pointer to the file system object */

CxSpiBusManager & SpiManager = CxSpiBusManager::getInstance();
...
static
bool move_window (
	unsigned long sector		/* Sector number to make apperance in the FatFs->win */
)						/* Move to zero only writes back dirty window */
{
	...
	if (wsect != sector) {	/* Changed current window */
		...
		if (fs->winflag) {	/* Write back dirty window if needed */
			if (SpiManager.sd_disk_write(fs->win, wsect, 1) != RES_OK) return false;
			fs->winflag = 0;
			if (wsect < (fs->fatbase + fs->sects_fat)) {	/* In FAT area */
				for (n = fs->n_fats; n >= 2; n--) {	/* Refrect the change to all FAT copies */
					wsect += fs->sects_fat;
					if (SpiManager.sd_disk_write(fs->win, wsect, 1) != RES_OK) break;
				}
			}
		}
#endif
		if (sector) {
			if (SpiManager.sd_disk_read(fs->win, sector, 1) != RES_OK) return false;
			fs->winsect = sector;
		}
	}
	return true;
...
static
unsigned long get_cluster (
	unsigned long clust			/* Cluster# to get the link information */
)
{
...
static
bool put_cluster (
	unsigned long clust,		/* Cluster# to change */
...
static
bool remove_chain (
	unsigned long clust			/* Cluster# to remove chain from */
)
{
	unsigned long nxt;
...
#ifndef _FS_READONLY
static
unsigned long create_chain (
	unsigned long clust			 /* Cluster# to stretch, 0 means create new */
)
{
...
static
unsigned long clust2sect (
	unsigned long clust		/* Cluster# to be converted */
)
{
	FATFS *fs = FatFs;
...
unsigned char check_fs (
	unsigned long sect		/* Sector# to check if it is a FAT boot record or not */
)
{
	...
	memset(fs->win, 0, 512);
	if (SpiManager.sd_disk_read(fs->win, sect, 1) == RES_OK) {	/* Load boot record */
...
#ifndef _FS_READONLY
static
unsigned char* reserve_direntry (
...
)
{
	...
	for (n = fs->sects_clust; n; n--) {
		if (SpiManager.sd_disk_write(fs->win, sector, 1) != RES_OK) return NULL;
		sector++;
	}
	fs->winflag = 1;
	return fs->win;
...
FRESULT check_mounted ()
{
	...
	if (SpiManager.sd_disk_status() & STA_NOINIT) {	/* The drive has not been initialized */
...
FRESULT f_mountdrv (void)
{
	...
	if (SpiManager.sd_disk_initialize() & STA_NOINIT)	return FR_NOT_READY;
...
#ifndef _FS_READONLY
	if ((mode & (FA_WRITE|FA_CREATE_ALWAYS|FA_OPEN_ALWAYS)) && (SpiManager.sd_disk_status() & STA_PROTECT))
...
)
{
	...
	*br = 0;
	if (!fs) return FR_NOT_ENABLED;
	if ((SpiManager.sd_disk_status() & STA_NOINIT) || !fs->fs_type) return FR_NOT_READY;	/* Check disk ready */
	...
	for ( ;  btr;									/* Repeat until all data transferred */
		...
		if ((fp->fptr % 512) == 0) {				/* On the sector boundary */
			...
			if (fp->flag & FA__DIRTY) {				/* Flush file I/O buffer if needed */
				if (SpiManager.sd_disk_write(fp->buffer, fp->curr_sect, 1) != RES_OK) goto fr_error;
				fp->flag &= ~FA__DIRTY;
			}
#endif
			...
			if (cc) {								/* Read maximum contiguous sectors directly */
				...
				if (SpiManager.sd_disk_read(rbuff, sect, cc) != RES_OK) goto fr_error;
				fp->sect_clust -= cc - 1;
				fp->curr_sect += cc - 1;
				rcnt = cc * 512; continue;
			}
			if (SpiManager.sd_disk_read(fp->buffer, sect, 1) != RES_OK)	/* Load the sector into file I/O buffer */
				goto fr_error;
		}
...
)
{
	...
	*bw = 0;
	if (!fs) return FR_NOT_ENABLED;
	if ((SpiManager.sd_disk_status() & STA_NOINIT) || !fs->fs_type) return FR_NOT_READY;
	if (fp->flag & FA__ERROR) return FR_RW_ERROR;	/* Check error flag */
	...
	for ( ;  btw;									/* Repeat until all data transferred */
		...
		if ((fp->fptr % 512) == 0) {				/* On the sector boundary */
			...
			if (fp->flag & FA__DIRTY) {				/* Flush file I/O buffer if needed */
				if (SpiManager.sd_disk_write(fp->buffer, fp->curr_sect, 1) != RES_OK) goto fw_error;
				fp->flag &= ~FA__DIRTY;
			}
			fp->curr_sect = sect;					/* Update current sector */
			...
			if (cc) {								/* Write maximum contiguous sectors directly */
				if (cc > fp->sect_clust) cc = fp->sect_clust;
				if (SpiManager.sd_disk_write(wbuff, sect, cc) != RES_OK) goto fw_error;
				fp->sect_clust -= cc - 1;
				fp->curr_sect += cc - 1;
				wcnt = cc * 512; continue;
			}
			if ((fp->fptr < fp->fsize) &&  			/* Fill sector buffer with file data if needed */
				(SpiManager.sd_disk_read(fp->buffer, sect, 1) != RES_OK))
					goto fw_error;
		}
...
FRESULT f_lseek (
	FIL *fp,		/* Pointer to the file object */
	unsigned long ofs		/* File pointer from top of file */
)
{
	unsigned long clust;
	unsigned char sc;
	...
	if (fp->flag & FA__DIRTY) {			/* Write-back dirty buffer if needed */
		if (SpiManager.sd_disk_write(fp->buffer, fp->curr_sect, 1) != RES_OK) goto fk_error;
		fp->flag &= ~FA__DIRTY;
	}
#endif
	...
	if (ofs) {
		...
		if (fp->fptr % 512) {										/* Load currnet sector if needed */
			if (SpiManager.sd_disk_read(fp->buffer, fp->curr_sect, 1) != RES_OK)
				goto fk_error;
		}
	}
...
)
{
	...
	if (!fs) return FR_NOT_ENABLED;
	if ((SpiManager.sd_disk_status() & STA_NOINIT) || !fs->fs_type)
		return FR_INCORRECT_DISK_CHANGE;
	...
	if (fp->flag & FA__WRITTEN) {
		...
		if (fp->flag & FA__DIRTY) {
			if (SpiManager.sd_disk_write(fp->buffer, fp->curr_sect, 1) != RES_OK) return FR_RW_ERROR;
			fp->flag &= ~FA__DIRTY;
		}
...
FRESULT f_getfree (
	unsigned long *nclust		/* Pointer to the double word to return number of free clusters */
)
{
...
)
{
	...
	if ((res = check_mounted()) != FR_OK) return res;
	if (SpiManager.sd_disk_status() & STA_PROTECT) return FR_WRITE_PROTECTED;
...
)
{
	...
	for (n = fs->sects_clust - 1; n; n--) {
		if (SpiManager.sd_disk_write(w, dsect+n, 1) != RES_OK) return FR_RW_ERROR;
	}
