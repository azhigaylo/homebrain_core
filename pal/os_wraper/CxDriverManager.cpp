...
CxDriverManager::CxDriverManager( ):
  driverCounter( 0 )  
{   
   mod_memset( DriverList, 0, sizeof(DriverList), sizeof(DriverList) );  
   //timer start
   //StartTCO();
}

CxDriverManager &CxDriverManager::getInstance( )
{
  static CxDriverManager theInstance;
  return theInstance;
}

//------------------------------------------------------------------------------

void CxDriverManager::StartTCO()
{
        // *****************************************************************************
        //   						timersetup.c
        //
        ...
        //    
        //     TIMER_CLOCK1 = MCK / 2     = 48054841 / 2     =  24027420 hz
        //     TIMER_CLOCK2 = MCK / 8     = 48054841 / 8     =  6006855hz
        //     TIMER_CLOCK3 = MCK / 32    = 48054841 / 32    =  1501713 hz
        //     TIMER_CLOCK4 = MCK / 128   = 48054841 / 128   =  375428 hz  
	...
	//	         011  TIMER_CLOCK4    (MCK/128  =    375428 hz)   <===== we select this one                                                 
	...
	//  CPCTRG   = 0  (RC Compare has no effect on the counter and its clock)   <===== we select this one  
	...
	//          11 (each edge of TIOA)
	//
	pTC->TC_CMR = 0x0003;				                        // TCCLKS = 1 (TIMER_CLOCK5)
	...
	//   31                              16 15                                    0    
	//	
        //    For exampel if we will use compare mode !!!!
        //
	...
	//
	pTC->TC_IER = 0x00;				                        // enable RC compare interrupt
	...
	//
	pTC->TC_IDR = 0xFF;				                        // disable all except RC compare interrupt
...
bool CxDriverManager::driverRegistration ( unsigned short DrvID, pIxDriver pDriver )
{
  bool result = false;

  // counter increment
  if( driverCounter < driverListSize )
  {
    DriverList[driverCounter].DrvID   = DrvID;
    DriverList[driverCounter].pDriver = pDriver;
...
bool CxDriverManager::loadDriver( portCHAR * pcName )
{
  bool result = false;  
  unsigned short curentDrvID = CRC16_T( pcName, mod_strlen(pcName, configMAX_TASK_NAME_LEN) );
  
  for( char iterator = 0; iterator < driverCounter; iterator++ )
  {
    if( DriverList[iterator].DrvID == curentDrvID )
    {
      pIxDriver pDriver = DriverList[iterator].pDriver;
...
void CxDriverManager::loadAllDrivers( )
{
  for( char iterator = 0; iterator < driverCounter; iterator++ )
  {
    pIxDriver pDriver = DriverList[iterator].pDriver;
